...
parser.add_argument('--epsilon', type=float, default=0.000001, help='avoid 0 gradient')
...
def SGD_Momentum(x,y,theta):
	...
	for i in range(0,MaxInteration):
		...
		#计算梯度，vi = Gama * vi-1(历史信息) + Alpha * gradient
		gradient = np.array([2 * loss, 2 * loss * x[RandomIndex]])
		v = GAMA * v + ALPHA * gradient
		Stopflag = (gradient[0]**2 + gradient[1]**2)**0.5
...
def SGD_TIPS(x,y,theta):
	...
	for i in range(0,MaxInteration):
		...
		if((gradient[0]*gradient[0] + gradient[1]*gradient[1])**0.5 < GradientStop):
			print(i,' gradient stop:',(gradient[0]*gradient[0] + gradient[1]*gradient[1])**0.5 )
			break
		#计算u v w delta_theta
		...
		print(i,' delta_theta',delta_theta)
...
def main():
	...
	theta = SGD_Momentum(traindata,trainlabel,theta)
